...
export const GET: APIRoute = async ({ request, locals }) => {
  ...
  return json({
    item: {
      ...
      previewUrl: item.type === 'image' ? mediaObjectUrl(item.storage_key_original, { width: 1600, fit: 'scale-down' }) : mediaObjectUrl(item.storage_key_preview),
      thumbnailUrl: item.type === 'image' ? mediaObjectUrl(item.storage_key_original, { width: 480, fit: 'cover' }) : mediaObjectUrl(item.storage_key_thumbnail),
